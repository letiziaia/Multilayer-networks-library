class AuxiliaryGraphBuilder(object):
    ...
    has_automorphism_group_generators = (
        False  # method can be used to generate automorphism groups
    )
    has_isomorphism_mapping = (
        False  # method can be used to generate an isomorphic mapping
    )
    ...
    def _assert_full_order(self, seq):
        for i in range(len(seq) - 1):
            assert (
                seq[i] < seq[i + 1]
            ), "Cannot sort the node or elemenetary layer names!"
    ...
    def compare_labels(self, other):
        assert (
            self.auxcolormap == other.auxcolormap
    ...
    def compare(self, other):
        ...
        assert (
            self.asp == other.asp and self.nasp == other.nasp
